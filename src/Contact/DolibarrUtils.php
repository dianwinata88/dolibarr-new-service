...
/**
 * Faithful ports of the Dolibarr helper functions the contact API depends
...
final class DolibarrUtils
{
    ...
    public function price2num(mixed $amount, string $rounding = '', string $option = ''): string
    {
        ...
        if ($option === 'auto') {
            // Convert everything to universal numeric. API context pins
            // SeparatorDecimal='.' and SeparatorThousand=',' upstream, so the
            // general dec/thousand branches reduce to this.
            $amount = str_replace(' ', '', (string) $amount); // avoid classic spaces
            $amount = preg_replace('/[a-zA-Z\/\*\\\\]/', '', $amount);
            $amount = str_replace(',', '', $amount); // strip thousand separators
        }
